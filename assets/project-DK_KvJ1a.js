import"./loader-bCRQtbAi.js";import{p as f,i as $}from"./apply-theme-Ce-kM_Nc.js";let s=null,u=0,o=[],m=null,y=!1;const p=()=>(s instanceof HTMLElement||(s=document.createElement("div"),s.className="gallery-lightbox",s.id="gallery-lightbox",s.setAttribute("role","dialog"),s.setAttribute("aria-modal","true"),s.setAttribute("aria-label","Image gallery"),s.setAttribute("aria-hidden","true"),s.hidden=!0,s.innerHTML=`
  ...
  `,document.title="Not Found — Projects"},B=(e,t)=>{var i,c;document.title=`${t.title} — Projects`;const a=t.status==="WIP"?" project-detail__status--wip":"",n=t.tags.map(l=>`<li>${l}</li>`).join(""),g=t.externalUrl?`<a
    ...
    ${P(r.images)}
    ${R(t.notes)}
  `,e.querySelectorAll("video").forEach((l,S)=>{U(l,S),M(l)}),(c=r.images)!=null&&c.length&&w(e,r.images)},C=()=>{const e=document.getElementById("project-detail"),t=A();if(!(e instanceof HTMLElement))return;const a=t?I(t):void 0,n=(a==null?void 0:a.theme)??f.defaultTheme;if($(f.themes,n,f.defaultTheme),!t){E(e);return}if(!a){E(e);return}B(e,a)};document.addEventListener("DOMContentLoaded",C);
