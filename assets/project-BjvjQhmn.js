import"./loader-bCRQtbAi.js";import{p as g,i as y}from"./apply-theme-Ce-kM_Nc.js";let l=null,o=0,i=[],p=null,h=!1;const c=()=>(l instanceof HTMLElement||(l=document.createElement("div"),l.className="gallery-lightbox",l.id="gallery-lightbox",l.setAttribute("role","dialog"),l.setAttribute("aria-modal","true"),l.setAttribute("aria-label","Image gallery"),l.setAttribute("aria-hidden","true"),l.hidden=!0,l.innerHTML=`
  ...
  `,document.body.appendChild(l),l.addEventListener("click",L)),l),b=()=>{const e=c(),t=i[o];if(!t)return;const a=e.querySelector(".gallery-lightbox__img"),n=e.querySelector(".gallery-lightbox__caption"),d=e.querySelector(".gallery-lightbox__counter"),s=e.querySelector('[data-action="prev"]'),r=e.querySelector('[data-action="next"]');if(!(a instanceof HTMLImageElement))return;a.src=t.src,a.alt=t.alt??"",n instanceof HTMLElement&&(n.textContent=t.caption??"",n.hidden=!t.caption),d instanceof HTMLElement&&(d.textContent=`${o+1} / ${i.length}`);const u=i.length>1;s instanceof HTMLButtonElement&&(s.hidden=!u),r instanceof HTMLButtonElement&&(r.hidden=!u)},x=e=>{if(!i.length)return;o=e,b();const t=c();t.hidden=!1,t.setAttribute("aria-hidden","false"),t.classList.add("is-open"),document.body.classList.add("gallery-lightbox-open");const a=t.querySelector(".gallery-lightbox__close");a instanceof HTMLElement&&a.focus()},_=()=>{const e=c();e.hidden=!0,e.setAttribute("aria-hidden","true"),e.classList.remove("is-open"),document.body.classList.remove("gallery-lightbox-open"),p instanceof HTMLElement&&(p.focus(),p=null)},f=e=>{i.length<=1||(o=(o+e+i.length)%i.length,b())},L=e=>{const t=e.target;if(!(t instanceof Element))return;const a=t.closest("[data-action]");if(!(a instanceof HTMLElement))return;const n=a.getAttribute("data-action");if(n==="close"){_();return}if(n==="prev"){f(-1);return}n==="next"&&f(1)},E=e=>{if(!c().hidden){if(e.key==="Escape"){e.preventDefault(),_();return}if(e.key==="ArrowLeft"){e.preventDefault(),f(-1);return}e.key==="ArrowRight"&&(e.preventDefault(),f(1))}},$=(e,t)=>{t!=null&&t.length&&(i=t,c(),h||(document.addEventListener("keydown",E),h=!0),e.querySelectorAll("[data-gallery-index]").forEach(a=>{a.addEventListener("click",()=>{const n=Number(a.getAttribute("data-gallery-index"));Number.isNaN(n)||(p=a instanceof HTMLElement?a:null,x(n))}),a.addEventListener("keydown",n=>{n instanceof KeyboardEvent&&(n.key!=="Enter"&&n.key!==" "||(n.preventDefault(),a.dispatchEvent(new Event("click"))))})}))},v=()=>new URLSearchParams(window.location.search).get("slug"),k=e=>g.projects.find(t=>t.slug===e),j=e=>{if(e.poster||e.readyState>=HTMLMediaElement.HAVE_CURRENT_DATA){e.classList.add("is-media-ready");return}e.addEventListener("loadeddata",()=>{e.classList.add("is-media-ready")},{once:!0})},H=e=>e!=null&&e.length?`
  ...
  `,document.title="Not Found — Projects"},A=(e,t)=>{var r;document.title=`${t.title} — Projects`;const a=t.status==="WIP"?" project-detail__status--wip":"",n=t.tags.map(u=>`<li>${u}</li>`).join(""),d=t.externalUrl?`<a
    ...
    ${T(s.images)}
    ${M(t.notes)}
  `,e.querySelectorAll("video").forEach(j),(r=s.images)!=null&&r.length&&$(e,s.images)},S=()=>{const e=document.getElementById("project-detail"),t=v();if(!(e instanceof HTMLElement))return;const a=t?k(t):void 0,n=(a==null?void 0:a.theme)??g.defaultTheme;if(y(g.themes,n,g.defaultTheme),!t){m(e);return}if(!a){m(e);return}A(e,a)};document.addEventListener("DOMContentLoaded",S);
